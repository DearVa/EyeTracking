#include "pch.h"
#include "EyeTracking.Windows.Capture.CypressCapture.h"
...
EyeTracking::Windows::Capture::CypressCapture::CypressCapture(IntPtr^ handle)
{
    this->header = new unsigned char [40];
    create_header(this->header, 480, 640);
    device = new CCyUSBDevice(handle->ToPointer());
}

void EyeTracking::Windows::Capture::CypressCapture::Start(Handler^ receiver)
{
    if (handler) return;
    handler = receiver;
    long length = 0;
    ...
    {
        looping = true;
        Threading::Tasks::Task::Run(gcnew Action(this, &CypressCapture::loop));
    }
}

void EyeTracking::Windows::Capture::CypressCapture::Stop()
{
    looping = false;
}

void EyeTracking::Windows::Capture::CypressCapture::abort_xfer_loop(
    const int pending,
    const bool opt,
...
void EyeTracking::Windows::Capture::CypressCapture::loop()
{
    int i;
    LONG length = 0;
    const auto ctrl_buf = new UCHAR;
    ...
    const auto ept = context->device->ControlEndPt; //USB	
...
void EyeTracking::Windows::Capture::CypressCapture::create_header(
